#!/usr/bin/env python3
"""Cloud Hunter — FINAL: RSI>50, no cooldown, all coins, 2x50%"""
import json, os, numpy as np, pandas as pd

COMM=0.002; MAX_SLIPPAGE=1.5; COOLDOWN=0  # تم الإلغاء
...
def trades(c,h,l,o,idx):
    ...
    for i in range(sk+sh,n):
        ...
        if s: sig+=1
        if pos:
            if h[i]>=ep*1.05: trs.append((eb,idx[i],tp-COMM*100));pos=0
            elif l[i]<=ep*0.975:
                pnl=max((c[i]/ep-1)*100-COMM*100,-sl*MAX_SLIPPAGE-COMM*100)
                trs.append((eb,idx[i],pnl));pos=0
        if not pos and s: pos=1;ep=c[i];eb=idx[i]
...
def run_pf(coin_trades):
    ...
    pnls=[p for _,_,p in sum([t for t,_ in coin_trades.values()],[])]
    ws=[p for p in pnls if p>0];ls=[p for p in pnls if p<0]
    return{'pnl':eq-1000,'dd':dd,'t':ex,'wr':wr,'aw':np.mean(ws) if ws else 0,'al':np.mean(ls) if ls else 0}

with open('/data/trading28/config/shariah_coins.json') as f: d=json.load(f)
ac=sorted(d['halal']+d['halal2'])

print("☁️ صياد السحابة — FINAL\n")
print("⚙️ 8h | 3/9/18 | TP=5% | SL=2.5% | RSI>50 | تبريد=0 | صفقتين×50%\n")
print(f"{'':>6s} | {'عملات':>4s} | {'إشارات':>5s} | {'منفذ':>4s} | {'WR':>5s} | {'ربح$':>8s} | {'سحب':>6s} | {'م.ربح':>6s} | {'م.خسارة':>6s}")
print("─"*72)
g=0;gt=0;gw=0
for pn,pdir in[('2023','2023'),('PREV','prev'),('CUR','1y')]:
    ...
    m=run_pf(ct)
    gt+=m['t'];gw+=int(m['wr']*m['t']/100)
    print(f"{pn:>6s} | {N:4d} | {ts:5d} | {m['t']:4d} | {m['wr']:4.1f}% | ${m['pnl']:+7,.0f} | {m['dd']:5.1f}% | {m['aw']:+5.2f}% | {m['al']:+5.2f}%")
    g+=m['pnl']
print(f"{'─'*72}")
print(f"💰    | {'':>4s} | {'':>5s} | {gt:4d} | {gw/gt*100:4.1f}% | ${g:+7,.0f}")
