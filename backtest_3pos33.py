#!/usr/bin/env python3
"""MAX_POS=3 — 33% لكل صفقة — أفضل 3 تركيبات"""
import json, numpy as np, pandas as pd, os, time, gc

COMM = 0.20; TF_MIN = 3; CAPITAL = 1000; MAX_POS = 3
...
CONFIGS = [
    ...
    ("TP0.8_SL0.5_TR3",  0.8, 0.5, 12, 0.03, 4, 0.10, 35, True),
]
...
# ═══════════════ محاكاة MAX_POS=3 ═══════════════
POS_PCT = 100/MAX_POS  # 33.33%
print(f"{'='*75}")
print(f"📊 MAX_POS={MAX_POS} — {POS_PCT:.0f}% لكل صفقة | تداخل زمني حقيقي")
print(f"{'='*75}")
...
for name,tp,sl,pl,trail,mh,whale,rsi,confirm in CONFIGS:
    ...
    # Also calculate fixed $333 per trade (no compounding)
    fixed_pnl = sum(p['pnl']/100*(1000/MAX_POS) for p in trades[:executed]) if executed > 0 else 0
    
    print(f"\n{'─'*60}")
    print(f"📊 {name} | TP={tp}% SL={sl}% TRAIL={trail}%")
    print(f"📋 إشارات: {len(trades):,} | ✅ منفذة: {executed:,} | ⏭️ متخطية: {skipped:,}")
    print(f"🟢 ربح: {wins:,} | 🔴 خسارة: {losses:,}")
    print(f"📈 WR: {wr:.1f}%")
    print(f"💵 إجمالي الربح: +{sum(p['pnl'] for p in trades if p['pnl']>0):.1f}% | 💸 إجمالي الخسارة: {sum(p['pnl'] for p in trades if p['pnl']<=0):.1f}%")
    print(f"🟢 متوسط ربح: +{avg_win:.2f}% | 🔴 متوسط خسارة: {avg_loss:.2f}%")
    print(f"📊 R:R: {rr:.2f}x | 📊 شارپ: {sharpe:.2f} | 📉 سحب: {max_dd:.1f}%")
    print(f"🏦 محفظة (تركيب {POS_PCT:.0f}%): $1,000 → ${equity:,.0f} (+{(equity/10-100):.1f}%)")
    print(f"💰 محفظة (بدون تركيب): $1,000 → ${1000+fixed_pnl:,.0f} (+{fixed_pnl/10:.1f}%)")
