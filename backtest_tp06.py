#!/usr/bin/env python3
"""TP=0.6% مع فلاتر مركب — MAX_POS=2 عالمي"""
...
# TP=0.6% — كل التركيبات مع فلتر مركب
CONFIGS = [
    ("TP0.6_SL0.5",  0.6, 0.5, 10, 0.05, 4, 0.10, 35, True),
    ("TP0.6_SL1.0",  0.6, 1.0, 12, 0.05, 4, 0.10, 35, True),
    ("TP0.6_SL1.5",  0.6, 1.5, 15, 0.10, 4, 0.10, 35, True),
    ("TP0.6_SL2.0",  0.6, 2.0, 15, 0.10, 4, 0.10, 35, True),
    ("TP0.6_SL0.5_لا_تريل", 0.6, 0.5, 10, 0.50, 4, 0.10, 35, True),  # تريل واسع = عملياً بدون تريل
]
...
# ═══════════════ محاكاة المحفظة ═══════════════
print(f"{'='*70}")
print(f"📊 TP=0.6% | فلتر مركب | MAX_POS=2 عالمي | close-only")
print(f"{'='*70}")
...
for name,tp,sl,pl,trail,mh,whale,rsi,confirm in CONFIGS:
    ...
    for s in range(MAX_POS):
        if slots[s] is not None:
            sex,spnl=slots[s]; pos_cap=equity*0.5; pnl_dollar=pos_cap*(spnl/100); equity+=pnl_dollar; slots[s]=None
    ...
    print(f"\n{'─'*60}")
    print(f"📊 {name} | TP={tp}% SL={sl}% PL={pl}% TRAIL={trail}% MH={mh}h")
    ...
    print(f"💰 صافي: {total_profit+total_loss:+.1f}%")
