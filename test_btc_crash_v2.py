#!/usr/bin/env python3
"""Cloud Hunter — 3Y: RSI>50 + BTC crash filter (index-aligned)"""
...
def compute_btc_state(c, h, l, o, ts, filter_type):
    """Returns array of bool: btc_ok[i] for each 8h candle"""
    ...
    for i in range(n):
        if filter_type == 'btc_ema200':
            ok[i] = not np.isnan(ema200[i]) and c8[i] > ema200[i]
        elif filter_type == 'btc_ema50':
            ok[i] = not np.isnan(ema50[i]) and c8[i] > ema50[i]
        elif filter_type == 'btc_rsi50':
            ok[i] = not np.isnan(rsi[i]) and rsi[i] > 50
...
def ichimoku_trades(c, h, l, o, idx, btc_ok=None):
    ...
    for i in range(senkou + shift, n):
        ...
        # BTC global filter by index
        if signal and btc_ok is not None and i < len(btc_ok):
            signal = signal and btc_ok[i]
...
btc_filters = [
    (None, 'RSI>50 فقط'),
    ('btc_ema200', 'RSI>50 + BTC>EMA200 8h'),
    ('btc_ema50', 'RSI>50 + BTC>EMA50 8h'),
    ('btc_rsi50', 'RSI>50 + BTC RSI>50 8h'),
    ('btc_ema200_rsi50', 'RSI>50 + BTC>EMA200+RSI>50 8h'),
]
...
for btc_flt, label in btc_filters:
    ...
    for pname, pdir in [('2023','2023'),('PREV','prev'),('CUR','1y')]:
        # Pre-compute BTC state for this period
        btc_ok = None
        if btc_flt:
            btc_data = load('BTCUSDT', pdir)
            if btc_data is not None:
                btc_ok = compute_btc_state(*btc_data, btc_flt)
