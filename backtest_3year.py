...
"""باك تيست 3 سنوات — أفضل 10 عملات — RSI<25"""
...
for sym in TOP10:
    ...
    days = len(df)*0.25/24
    print(f'  {sym:<10} {sym_trades:>4}ت | WR {sym_wr:.0f}% | {days:.0f} يوم', flush=True)
...
print(f'\n{"="*60}')
print(f'📊 إجمالي 10 عملات — RSI<25')
print(f'{"="*60}')
...
for sym in TOP10:
    ...
    c = coin_stats[sym]
    wr = c['w']/c['t']*100
    print(f'  {sym:<10} {c["t"]:>3}ت | WR {wr:.0f}% | صافي {c["pnl"]:+.1f}%')
...
for yr in [2023,2024,2025,2026]:
    ...
    print(f'  {yr}: {len(yt)} صفقة | WR {yw/len(yt)*100:.0f}%')
