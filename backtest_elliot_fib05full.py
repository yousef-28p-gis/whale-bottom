#!/usr/bin/env python3
"""Elliot 5-Wave — TP = Fib 0.5 only (full position, no half, no BE)"""
...
def simulate(close,high,low,coin,w5_filter):
    ...
    for H1,L1,H2,L2,H3,L3 in pats:
        ...
        tp_fib05 = L3[1] + 0.5*w5_size  # Fib 0.5
        if tp_fib05 <= ep: continue
        ...
        for j in range(eb+1, min(n,eb+TIME_BARS+1)):
            ...
            if bh >= tp_fib05:
                exit_idx=j; exit_type='TP'; exit_pnl=(tp_fib05/ep-1)*100-COMM; break
            if bc <= sl:
...
print('═══ Elliot 5-Wave: بيع كامل عند Fib 0.5 ═══', flush=True)
...
print(f'📊 بيانات: {len(COINS)} عملة — ⏱️ TP=Fib 0.5 كامل')
...
print(f'✅ {len(execd)} ⏭️ {len(all_t)-len(execd)} | 🎯TP {tp_c} 🛑SL {sl_c} ⏱️TIME {time_c}')
