#!/usr/bin/env python3
"""Cloud Hunter — 3Y: RSI>50 + ATR-based whale filters"""
...
def ichimoku_trades(c, h, l, o, idx, flt=None, whale_flt=None):
    ...
    for i in range(senkou + shift, n):
        ...
        # Whale ATR/range filter
        if signal and whale_flt:
            if whale_flt == 'range_gt_atr':     signal = signal and candle_range[i] > atr_pct[i]
            elif whale_flt == 'range_gt_1.5atr': signal = signal and candle_range[i] > atr_pct[i]*1.5
            elif whale_flt == 'range_gt_2atr':   signal = signal and candle_range[i] > atr_pct[i]*2
            elif whale_flt == 'range_gt_3pct':   signal = signal and candle_range[i] > 3
            elif whale_flt == 'range_gt_5pct':   signal = signal and candle_range[i] > 5
            elif whale_flt == 'body_gt_60':      signal = signal and body_pct[i] > 0.6
            elif whale_flt == 'body_gt_80':      signal = signal and body_pct[i] > 0.8
            elif whale_flt == 'big_range_big_body': signal = signal and candle_range[i] > atr_pct[i]*1.5 and body_pct[i] > 0.6
...
filters = [
    ('rsi50', None, 'RSI>50 فقط'),
    ('rsi50', 'range_gt_atr', 'RSI>50 + مدى>ATR'),
    ('rsi50', 'range_gt_1.5atr', 'RSI>50 + مدى>1.5×ATR'),
    ('rsi50', 'range_gt_2atr', 'RSI>50 + مدى>2×ATR'),
    ('rsi50', 'range_gt_3pct', 'RSI>50 + مدى>3%'),
    ('rsi50', 'range_gt_5pct', 'RSI>50 + مدى>5%'),
    ('rsi50', 'body_gt_60', 'RSI>50 + جسم>60%'),
    ('rsi50', 'body_gt_80', 'RSI>50 + جسم>80%'),
    ('rsi50', 'big_range_big_body', 'RSI>50 + مدى>1.5ATR+جسم>60%'),
]

print(f"☁️ صياد السحابة | 3 سنوات | كل العملات | صفقتين × $500\n")

for flt, whale_flt, label in filters:
    print(f"📋 {label}")
    ...
    for pname, pdir in [('2023','2023'),('PREV','prev'),('CUR','1y')]:
        ...
        for sym in all_coins:
            ...
            trades, signals = ichimoku_trades(c8, h8, l8, o8, idx, flt=flt, whale_flt=whale_flt)
            if len(trades) >= 3:
