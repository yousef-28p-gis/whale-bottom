...
"""باك تيست 5 سنوات — 50 عملة — حوت القاع"""
...
print(f'🔍 50 عملة | 5 سنوات | صفقتين×50%')
print()
...
for fname in sorted(os.listdir(CACHE_DIR)):
    ...
    if sym_trades > 0 or done % 10 == 0:
        ...
        print(f'  {done:>2}/50 {sym:<8} {sym_trades:>4}ت | WR {sym_wr:.0f}%', flush=True)
...
print(f'\n{"="*60}')
print(f'📊 50 عملة — 5 سنوات — RSI<25')
print(f'{"="*60}')
...
for sym, c in sorted(coin_stats.items(), key=lambda x: x[1]['pnl'], reverse=True)[:10]:
    wr = c['w']/c['t']*100
    print(f'  {sym:<8} {c["t"]:>3}ت | WR {wr:.0f}% | صافي {c["pnl"]:+.1f}%')
...
for yr in [2021,2022,2023,2024,2025,2026]:
    ...
    print(f'  {yr}: {len(yt):>3} صفقة | WR {yw/len(yt)*100:.0f}%')
