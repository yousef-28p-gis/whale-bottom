...
"""Config C: RSI<20, whale>=0.50, skip 10 worst coins — 5-year backtest"""
...
print(f'⚙️ Config C: TP={TP} SL={SL} PL={PL} TR={TRAIL} MH={MH}h WHALE≥{WHALE_MIN} RSI<{RSI_MAX}')
print(f'🔍 Skip 10 worst coins | 5 years | 2×50% portfolio')
print()

all_trades = []
done = 0; skipped_coins = 0
...
for fname in sorted(os.listdir(CACHE_DIR)):
    ...
    print(f'  {done:>2}/40 {sym:<8} {sym_trades:>4} signals', flush=True)

print(f'\nSkipped {skipped_coins} coins: {", ".join(sorted(SKIP_COINS))}')
print(f'Processed {done} coins')
...
print(f'\n{"="*60}')
print(f'📊 SIGNAL STATISTICS — Config C (RSI<{RSI_MAX})')
print(f'{"="*60}')
...
print('\n🏆 Top 10 coins:')
for sym, c in sorted(coin_stats.items(), key=lambda x: x[1]['pnl'], reverse=True)[:10]:
    wr = c['w']/c['t']*100
    print(f'  {sym:<8} {c["t"]:>3}t | WR {wr:.0f}% | Net {c["pnl"]:+.1f}%')

print('\n💀 Worst 10 coins:')
for sym, c in sorted(coin_stats.items(), key=lambda x: x[1]['pnl'])[:10]:
    wr = c['w']/c['t']*100
    print(f'  {sym:<8} {c["t"]:>3}t | WR {wr:.0f}% | Net {c["pnl"]:+.1f}%')
...
print(f'\n{"="*60}')
print(f'💼 PORTFOLIO — 2×50% — 5 Years — Config C (RSI<{RSI_MAX})')
print(f'{"="*60}')
print(f'📋 Total signals: {len(all_trades)}')
print(f'✅ Executed: {taken} | ⏭️ Skipped: {skipped}')
print(f'🟢 Exec wins: {exec_wins} | 🔴 Exec losses: {taken-exec_wins}')
print(f'📈 Exec WR: {exec_wins/taken*100:.1f}%' if taken>0 else 'N/A')
print(f'💼 Final capital: ${capital:,.0f} (from $1,000) — {capital/10-100:+.1f}%')
...
for yr in [2021,2022,2023,2024,2025,2026]:
    ...
    ynet = sum(t['pnl'] for t in yt)
    print(f'  {yr}: {len(yt):>3} trades | WR {yw/len(yt)*100:.0f}% | Net {ynet:+.1f}%')

# Monthly 2026
print('\n📅 2026 Monthly:')
for m in range(1,8):
    mt = [t for t in exec_trades if t['dt'].year==2026 and t['dt'].month==m]
    if not mt: continue
    mw = sum(1 for t in mt if t['pnl']>0)
    mnet = sum(t['pnl'] for t in mt)
    print(f'  {2026}-{m:02d}: {len(mt):>3} trades | WR {mw/len(mt)*100:.0f}% | Net {mnet:+.1f}%')
