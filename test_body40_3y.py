#!/usr/bin/env python3
"""Cloud Hunter — 3Y test: No filter vs جسم>40%"""
...
def ichimoku_trades(c, h, l, o, idx, use_body_filter=False):
    ...
    for i in range(senkou + shift, n):
        ...
        raw_signal = above and golden
        
        # Apply body filter
        signal = raw_signal
        if use_body_filter:
            signal = raw_signal and body_pct[i] > 0.4
        if signal: signals += 1
...
# Rank by total PnL (without filter)
coin_pnls = {}
for pdir in ['2023','prev','1y']:
    for sym in tradeable:
        ...
        trades, _ = ichimoku_trades(c8, h8, l8, o8, idx, use_body_filter=False)
        if len(trades) >= 3:
...
for use_filter, label in [(False, 'بدون فلتر'), (True, 'جسم > 40%')]:
    print(f"📋 {label}")
    ...
    for pname, pdir in [('2023','2023'),('PREV','prev'),('CUR','1y')]:
        ...
        for sym in clean:
            ...
            if resampled is None: continue
            c8, h8, l8, o8, idx = resampled
            trades, signals = ichimoku_trades(c8, h8, l8, o8, idx, use_body_filter=use_filter)
            if len(trades) >= 3:
    ...
    print(f"{'─'*68}")
    print(f"💰 المجموع | {'':>4s} | {grand_sig:5d} | {'':>4s} | {'':>5s} | ${grand:+7,.0f}\n\n")
