#!/usr/bin/env python3
"""Compare Hunter Whale backtest vs live-style simulation on June 2026."""
...
def simulate_backtest(df, entry_idx):
    """Original: check high/low of each candle (intra-candle)"""
    ...
    for j in range(entry_idx + 1, len(df)):
        ...
        if hours > MAX_H:
            return ('⏰', round((row['close'] - entry_price) / entry_price * 100, 4))
        ...
        if pl_triggered:
            ...
            if row['low'] <= trail_price:
                return ('🐌', round((trail_price - entry_price) / entry_price * 100, 4))
    
    return ('📦', round((df.iloc[-1]['close'] - entry_price) / entry_price * 100, 4))

def simulate_live(df, entry_idx):
    """Live-style: check close price only (simulating check_position every 15m)"""
    entry_price = df.iloc[entry_idx]['close']
    tp_price = entry_price * (1 + TP/100)
    sl_price = entry_price * (1 - SL/100)
    pl_price = entry_price + (tp_price - entry_price) * (PL / 100)
    ...
    for j in range(entry_idx + 1, len(df)):
        row = df.iloc[j]
        hours = (j - entry_idx) * 0.25
        current = row['close']  # ← live uses close price only
        
        if hours > MAX_H:
            return ('⏰', round((current - entry_price) / entry_price * 100, 4))
...
for s in raw:
    ...
    if dt.month != 6 or dt.year != 2026: continue
    signals.append({'symbol': s['symbol'], 'dt': dt, 'month': dt.strftime('%Y-%m')})

print(f'June signals: {len(signals)}')
...
for (sym, mon), sigs in sorted(by_pair.items()):
    ...
    for sig in sigs:
        ...
        for j, row in forward.iterrows():
            if j * 0.25 > 24: break
            if row['entry']:
                wi = j; wv = float(row['whale']); break
        
        if wi is None or wv < WHALE_MIN: continue
        
        # Backtest simulation (high/low)
        status_bt, pnl_bt = simulate_backtest(forward, wi)
        trades_bt.append({
            'symbol': sym, 'dt': sig['dt'], 'pnl': pnl_bt,
            'net': round(pnl_bt - COMMISSION, 4), 'exit': status_bt
        })
        
        # Live simulation (close only)
        status_lv, pnl_lv = simulate_live(forward, wi)
        trades_live.append({
            'symbol': sym, 'dt': sig['dt'], 'pnl': pnl_lv,
            'net': round(pnl_lv - COMMISSION, 4), 'exit': status_lv
        })
...
def stats(trades, name):
    ...
    print(f'  إغلاقات: 🎯={exits.get("🎯",0)} 🛑={exits.get("🛑",0)} 🐌={exits.get("🐌",0)} ⏰={exits.get("⏰",0)}')
    
    return wr, net

wr_bt, net_bt = stats(trades_bt, 'الحوت الأصلي (هاي/لو داخل الشمعة)')
wr_lv, net_lv = stats(trades_live, 'الحوت الحي (سعر الإغلاق فقط)')

# ── Side-by-side differences ──
print(f'\n{"="*50}')
print(f'🔍 مقارنة الصفقات المختلفة')
print(f'{"="*50}')
diff_count = 0
for bt, lv in zip(trades_bt, trades_live):
    if bt['exit'] != lv['exit'] or abs(bt['pnl'] - lv['pnl']) > 0.01:
        diff_count += 1
        if diff_count <= 10:
            print(f'  {bt["symbol"]:<10} | أصلي: {bt["exit"]} {bt["pnl"]:+.2f}% | حي: {lv["exit"]} {lv["pnl"]:+.2f}% | {bt["dt"]}')

print(f'\nإجمالي الصفقات المختلفة: {diff_count}/{len(trades_bt)}')

# ── Portfolio simulation for both ──
def portfolio(trades):
    cap = 1000; active = []; peak = 1000; max_dd = 0
    trades_sorted = sorted(trades, key=lambda t: t['dt'])
...
cap_bt, dd_bt = portfolio(trades_bt)
cap_lv, dd_lv = portfolio(trades_live)

print(f'\n💼 محفظة:')
print(f'  أصلي: $1000 → ${cap_bt:.0f} (DD {dd_bt:.1f}%)')
print(f'  حي:   $1000 → ${cap_lv:.0f} (DD {dd_lv:.1f}%)')
