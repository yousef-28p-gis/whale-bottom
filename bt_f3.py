...
valid=[f for f in files if os.path.exists(f'{CACHE_DIR}/{f}')]
...
for t in et: exits[t['exit']]+=1
aw=sum(n for n in en if n>0)/max(1,sum(1 for n in en if n>0))
al=sum(n for n in en if n<0)/max(1,sum(1 for n in en if n<0))

print(f'\n✅ فلتر 3: {len(trades)} إشارة | {taken} منفذة | WR {ew/taken*100:.1f}%')
print(f'💼 $1000 → ${cap:.0f} (+{(cap/1000-1)*100:.1f}%) | DD {dd:.2f}% | سنوي {((cap/1000)**(1/5)-1)*100:+.1f}%')
print(f'🟢 متوسط ربح: +{aw:.2f}% | 🔴 متوسط خسارة: {al:.2f}%')
print(f'📤 TP={exits.get("TP",0)} SL={exits.get("SL",0)} TRAIL={exits.get("TRAIL",0)} TIME={exits.get("TIME",0)+exits.get("EOD",0)}', flush=True)

with open('/tmp/bt_f3.json','w') as f: json.dump(trades,f)
print('💾 /tmp/bt_f3.json', flush=True)
