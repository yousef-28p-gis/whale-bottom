...
def run_mode(mode, max_coins=50):
    trades=[]; done=0
    files=[f for f in sorted(os.listdir(CACHE_DIR)) if f.endswith('.json')]
    valid=[fn for fn in files if os.path.exists(f'{CACHE_DIR}/{fn}')][:max_coins]
    total=len(valid)
    print(f'  [{mode}] {total} عملة...', flush=True)
    for fname in valid:
        ...
        if done%10==0: print(f'    {done}/{total} | {len(trades)} صفقة', flush=True)
    return trades

def portfolio_stats(trades, label):
    if not trades: return {}
    nets=[t['pnl'] for t in trades]; wins=sum(1 for n in nets if n>0)
    ...
    for t in sorted_t:
        ...
        pnl_amt=ps*t['pnl']/100; capital-=ps
        active.append((dt+timedelta(hours=MH),ps,pnl_amt)); taken+=1; exec_t.append(t)
    ...
    return {'label':label,'signals':len(trades),'executed':taken,'skipped':skipped,
            'wr':exec_wins/taken*100 if taken else 0,'net':sum(exec_nets),'avg_win':avg_w,
            'avg_loss':avg_l,'ev':sum(exec_nets)/taken if taken else 0,
...
for mode,label in [('base','بدون'),('f2','فلتر2'),('f3','فلتر3'),('combo','2+3')]:
    print(f'\n🐋 {label}...', flush=True)
    trades=run_mode(mode, 50)
    stats=portfolio_stats(trades, label)
    results[mode]=stats
    print(f'  ✅ {stats["executed"]} صفقة | WR {stats["wr"]:.1f}% | عائد {stats["pf_ret"]:.1f}% | DD {stats["max_dd"]:.2f}%', flush=True)

print('\n'+'='*70)
print('📊 مقارنة — 50 عملة حلال — 5 سنوات')
print('='*70)
print(f'{"":<22} {"بدون":>10} {"فلتر2":>10} {"فلتر3":>10} {"2+3":>10}')
print('-'*60)
for key,desc in [('signals','📋 إشارات'),('executed','✅ منفذة'),('wr','📈 Win Rate%'),('avg_win','🟢 متوسط ربح%'),('avg_loss','🔴 متوسط خسارة%'),('pf_ret','💼 عائد المحفظة%'),('max_dd','📉 أقصى سحب%'),('annual','📈 عائد سنوي%')]:
    vals=[f'{results[m][key]:.1f}' if isinstance(results[m][key],float) else str(results[m][key]) for m in ['base','f2','f3','combo']]
...
print(f'\n🏆 أفضل WR: {best_wr[1]["label"]} — WR {best_wr[1]["wr"]:.1f}% | عائد {best_wr[1]["pf_ret"]:.1f}% | DD {best_wr[1]["max_dd"]:.2f}%')
print(f'🏆 أفضل عائد: {best_pf[1]["label"]} — WR {best_pf[1]["wr"]:.1f}% | عائد {best_pf[1]["pf_ret"]:.1f}% | DD {best_pf[1]["max_dd"]:.2f}%')
print('='*70)

with open('/tmp/filter_results.json','w') as f:
    json.dump({m:{k:v if isinstance(v,(int,float)) else str(v) for k,v in s.items() if k!='exits'} for m,s in results.items()}, f, indent=2)
print('\n✅ /tmp/filter_results.json', flush=True)
