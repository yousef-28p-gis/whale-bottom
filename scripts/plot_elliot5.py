#!/usr/bin/env python3
"""Plot 10 Elliot 5-Wave patterns (2 pages x 5)"""
...
def find_5waves(pv):
    ...
    for i in range(len(pv)-5):
        ...
        if w2>=w1 or w3<=min(w1,w5) or H3>=L1 or L3>=L2: continue
...
print(f'Total: {len(all_pats)} patterns')
random.seed(42)
picks = random.sample(all_pats, 10)
...
for page in range(2):
    ...
    for idx, (cn, pat) in enumerate(page_picks):
        ...
        for wi, (pi, pr, pt) in enumerate(waves):
            ...
            if 0 <= x < win:
                ...
                ax.annotate(labels[wi], (x, pr*offset), fontsize=11, fontweight='bold', ha='center', color=colors[wi])
        ...
        ax.set_title(f'{cn} | w1:{w1:.4f}  w2:{w2:.4f}  w3:{w3:.4f}  w4:{w4:.4f}  w5:{w5:.4f}',
    ...
    plt.suptitle(f'Elliot 5-Wave Impulse Patterns ({page+1}/2) | Rules: W2<W1, W3 shortest, W4<W1, Lower lows',
        fontsize=14, fontweight='bold', y=0.998, color='black')
    plt.tight_layout()
    out = f'/data/trading28/scripts/elliot5_page{page+1}.png'
