#!/usr/bin/env python3
"""Cloud Hunter — 3Y: RSI>50 + position sizing variants"""
...
def run_portfolio(coin_trades, max_positions, position_pct):
...
configs = [
    (2, 0.50, 'RSI>50 + صفقتين×50%'),
    (1, 1.00, 'RSI>50 + صفقة×100%'),
    (1, 0.50, 'RSI>50 + صفقة×50%'),
    (3, 0.33, 'RSI>50 + 3 صفقات×33%'),
    (4, 0.25, 'RSI>50 + 4 صفقات×25%'),
]

print(f"☁️ صياد السحابة | 3 سنوات | كل العملات\n")

for max_pos, pos_pct, label in configs:
    print(f"📋 {label}")
    ...
    for pname, pdir in [('2023','2023'),('PREV','prev'),('CUR','1y')]:
        ...
        m = run_portfolio(coin_trades, max_pos, pos_pct)
