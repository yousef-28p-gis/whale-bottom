#!/usr/bin/env python3
"""
⚡ صياد البرق — بوت حي 3m
يمسح 143 عملة حلال كل 3 دقائق (متزامن مع حدود 3-دقائق UTC)

╔══════════════════════════════════════════════════════════════╗
║  ⛔ قوانين ثابتة — ممنوع التعديل بدون تأكيد صريح من يوسف:  ║
║                                                              ║
║  1. CLOSE-ONLY للخروج: SL + TRAIL ينتظران إغلاق الشمعة       ║
║  2. SL حقيقي: (close/entry-1)*100 - 0.2% — غير محصور          ║
║  3. MAX_POS=2 عالمي — ليس لكل عملة                            ║
║  4. دخول بسعر التيكر الحي (get_live_price)                    ║
║  5. المؤشرات: whale(7-layer) + spike + RSI(14)                ║
║  6. الدخول: WHALE≥0.10, SPIKE≥1.5, RSI<35, تأكيد أخضر         ║
║  7. الخروج: TP(تيكر)/SL(إغلاق)/TRAIL(PL=12%)/TIME(4h)         ║
...
║    - TP, SL, TRAIL, MAX_H (فقط بطلب يوسف)                     ║
╚══════════════════════════════════════════════════════════════╝
"""
...
# ═══════════════ الإعدادات ═══════════════
SYMBOLS_CACHE_FILE = '/data/trading28/cache/khatef_live/'
STATE_FILE = '/data/trading28/khatef_state.json'
REPORT_FILE = '/data/trading28/khatef_report.txt'
LOG_FILE = '/data/trading28/khatef_log.txt'
...
def check_entry(df, i):
    """Check if candle i is a valid entry"""
...
def sleep_until_next_3m_boundary():
    """ينام حتى بداية شمعة 3 دقائق التالية (00:00, 00:03, 00:06 UTC)"""
...
def main():
    ...
    while True:
        ...
        # منع الدخولات المكررة: تتبع (symbol, entry_bar_ts) المستخدمة
        used_entries = set()
        ...
        for coin in COINS:
            ...
            # Check last 3 candles for entry
            for i in range(max(50, len(df) - 4), len(df) - 1):
                if check_entry(df, i):
                    # دخول بسعر التيكر الحي — مش سعر إغلاق الكاش
                    ...
                    if len(state['active_positions']) < MAX_POS:
                        ...
                        report_lines.append(
                            f'🟢 صفقة دخول {coin}\n'
                            f'   LONG : {entry_price:.6f}\n'
                            f'   target: {pos["tp"]:.6f}\n'
                            f'   STOP-LOSS: {pos["sl"]:.6f}'
                        )
        ...
        for pos in state['active_positions'][:]:
            ...
            # SL — تأكيد بإغلاق شمعة (close-only)
                ...
                # إذا الشمعة ما أغلقت تحت SL → لا تخرج
            # PL + Trail
            ...
            if exit_reason:
                ...
                closed_this_scan.append(
                    f'صفقة خروج {symbol} | {exit_reason} | {exit_pnl:+.2f}% | '
                    f'⏱️ {hold_minutes:.0f}د'
                )
                log(f'EXIT: {symbol} {exit_reason} {exit_pnl:+.2f}%')
        
        # Build report
        if new_entries > 0 or closed_this_scan:
            report = '⚡ صياد البرق — تحديث\n'
            report += '━' * 35 + '\n'
            ...
            if active_count > 0:
                report = f'⚡ صياد البرق | {active_count}/{MAX_POS} صفقات مفتوحة\n'
                report += '━' * 35 + '\n'
                report += f'\n📌 صفقات مفتوحة:\n'
                for pos in state['active_positions']:
                    sym = pos['symbol']
                ...
                write_report(f'😴 لا جديد | ⚡ صياد البرق | {len(COINS)} عملة')
