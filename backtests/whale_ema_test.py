#!/usr/bin/env python3
"""Test different daily EMAs for market filter"""
...
print(f"{'فلتر':<12} {'إشارات':>6} {'صفقات':>6} {'WR':>5} {'محفظة':>8} {'DD':>6} {'R:R':>5}")
print("-"*55)

for ema_p in [20,50,100,200]:
    ema_map=ddf.set_index('date')[f'ema{ema_p}'].to_dict()
    df['ema']=df['date'].map(ema_map)
    market_ok=df['close']>df['ema']
    entry_sig=df['spike'] & trend_ok & market_ok
    ...
    sigs=len(eis)
    print(f"EMA{ema_p} اليومي   {sigs:>6} {n:>6} {wr:>4.0f}% ${equity:>7,.0f} {dd.min():>5.1f}% {rr:>4.1f}x")
