#!/usr/bin/env python3
"""Cloud Hunter — 3Y: RSI>50 + whale volume filters"""
...
def load(sym, period):
    ...
    return (np.array(j['c'],float), np.array(j['h'],float), np.array(j['l'],float),
            np.array(j['o'],float), np.array(j.get('v',[0]*len(j['c'])),float), j.get('ts',[]))

def resample_8h(c, h, l, o, v, ts):
    try:
        idx = pd.to_datetime(np.array(ts), unit='ms')
        df = pd.DataFrame({'o':o,'h':h,'l':l,'c':c,'v':v}, index=idx)
        r = df.resample('8h').agg({'o':'first','h':'max','l':'min','c':'last','v':'sum'}).dropna()
        return r['c'].values, r['h'].values, r['l'].values, r['o'].values, r['v'].values, r.index
    except: return None
...
def ichimoku_trades(c, h, l, o, vol, idx, flt=None, whale_flt=None):
    ...
    vol_ma50 = pd.Series(vol).rolling(50).mean().values
    ...
    for i in range(senkou + shift, n):
        ...
        if signal and whale_flt:
            if whale_flt == 'vol1.5x':   signal = signal and vol[i] > vol_ma20[i]*1.5
            elif whale_flt == 'vol2x':   signal = signal and vol[i] > vol_ma20[i]*2
            elif whale_flt == 'vol3x':   signal = signal and vol[i] > vol_ma20[i]*3
            elif whale_flt == 'vol5x':   signal = signal and vol[i] > vol_ma20[i]*5
            elif whale_flt == 'vol2x_50': signal = signal and vol[i] > vol_ma50[i]*2
...
filters = [
    (None, None, 'RSI>50 فقط'),
    ('rsi50', 'vol1.5x', 'RSI>50 + حجم>1.5x'),
    ('rsi50', 'vol2x', 'RSI>50 + حجم>2x'),
    ('rsi50', 'vol3x', 'RSI>50 + حجم>3x'),
    ('rsi50', 'vol5x', 'RSI>50 + حجم>5x'),
    ('rsi50', 'vol2x_50', 'RSI>50 + حجم>2x50'),
]
...
for flt, whale_flt, label in filters:
    ...
    for pname, pdir in [('2023','2023'),('PREV','prev'),('CUR','1y')]:
        ...
        for sym in all_coins:
            ...
            c8, h8, l8, o8, v8, idx = resampled
            if len(v8) < 50: continue
            trades, signals = ichimoku_trades(c8, h8, l8, o8, v8, idx, flt=flt, whale_flt=whale_flt)
            if len(trades) >= 3:
