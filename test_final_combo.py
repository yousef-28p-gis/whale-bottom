#!/usr/bin/env python3
"""Cloud Hunter — final combo: RSI>50 + 4h>EMA50 + 3x33% + portfolio CB"""
...
def trades(c,h,l,o,idx,trend_flt=None):
    ...
    ri=rsi(c);ema4h50=pd.Series(c).ewm(span=25,adjust=False).mean().values
    ...
    for i in range(sk+sh,n):
        ...
        if s and trend_flt=='4h_ema50': s=s and c[i]>ema4h50[i]
        if s: sig+=1
        if pos:
...
def run_cb(coin_trades,max_pos,pos_pct,loss_limit,pause_h):
    eq=1000;cv=[1000];op={};tl=[]
    for sym,(trs,_) in coin_trades.items():
        for et,xt,pnl in trs:
            em=et.value//10**6 if hasattr(et,'value') else et
            xm=xt.value//10**6 if hasattr(xt,'value') else xt
            tl.append((em,'entry',sym,pnl));tl.append((xm,'exit',sym,pnl))
    tl.sort();ex=0;wi=0;sk=0;cl=0;pu=0
    for t,ty,sym,pnl in tl:
        if ty=='entry':
            if t<pu: sk+=1;continue
            if len(op)<max_pos: op[sym]=eq*pos_pct
        elif ty=='exit':
            if sym in op:
    ...
    return{'pnl':eq-1000,'dd':dd,'trades':ex,'wr':wr,'eq':eq,'skipped':sk}

with open('/data/trading28/config/shariah_coins.json') as f: d=json.load(f)
ac=sorted(d['halal']+d['halal2'])

configs=[
    (2,0.50,0,0,None,'RSI>50 + صفقتين×50%'),
    (2,0.50,0,0,'4h_ema50','RSI>50 + 4h>EMA50 + صفقتين×50%'),
    (3,0.33,2,24,'4h_ema50','RSI>50 + 4h>EMA50 + 3×33% + تبريد 24h'),
    (2,0.50,2,24,'4h_ema50','RSI>50 + 4h>EMA50 + صفقتين + تبريد 24h'),
]

print(f"☁️ صياد السحابة | 3 سنوات | كل العملات\n")
for mx,pc,ll,ph,flt,label in configs:
    print(f"📋 {label}")
    print(f"{'':>6s} | {'عملات':>4s} | {'إشارات':>5s} | {'منفذ':>4s} | {'WR':>5s} | {'ربح$':>8s} | {'سحب':>6s} | {'نهائي$':>8s}")
    print("─"*68)
    g=0;gs=0;gt=0
    for pn,pd in [('2023','2023'),('PREV','prev'),('CUR','1y')]:
        ct={}
        for s in ac:
            d_=load(s,pd)
            ...
            c8,h8,l8,o8,ix=rp
            if len(c8)<600: continue
            trs,sig=trades(c8,h8,l8,o8,ix,trend_flt=flt)
            if len(trs)>=3: ct[s]=(trs,sig)
        N=len(ct);ts=sum(v[1] for v in ct.values())
        m=run_cb(ct,mx,pc,ll,ph)
        gs+=ts;gt+=m['trades']
        print(f"{pn:>6s} | {N:4d} | {ts:5d} | {m['trades']:4d} | {m['wr']:4.1f}% | ${m['pnl']:+7,.0f} | {m['dd']:5.1f}% | ${m['eq']:7,.0f}")
        g+=m['pnl']
    print(f"{'─'*68}")
    print(f"💰 | {'':>4s} | {gs:5d} | {gt:4d} | {'':>5s} | ${g:+7,.0f}\n\n")
